...
class Jwt extends \RESTling\Security {
    ...
    public function validate($model, $input) {
        ...
        if ($input->hasParameter("Authorization", "header")) {
            ...
            if (count($aAuth) == 2 && $aAuth[0] === "Bearer") {
                // validate the token
                $this->validateToken($model, $aAuth[1]);
            }
        }
    }
    ...
    /**
 	 * Validates a JWT.
     *
     * The detachedPayload allows to evaluate the JWT using an external payload
     * object. This is presently unused.
 	 *
 	 * @param \RESTling\Model $model
     * @param mixed $token - holds the token to validate
     * @param array $detachedPayload - default null
 	 * @return void
	 */
	public function validateToken($model, $token, $detachedPayload = null) {
        ...
        // keep the final JWT for further inspection
        $this->jwt = $jwt;
        $self->success();
    }
    ...
    public function getClaim($claimName) {
        if ($this->jwt) {
            return $this->jwt->getClaim($claimName);
        }
        return null;
    }
}
